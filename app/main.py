...
summ_cols = ['Site', 'Measurement', 'total count', 'DTL count', 'start date', 'end date', 'min', 'median', 'mean', 'max', 'standard deviation']
...
def get_results(base_url, hts, mtype, ref):
    ...
    ### Make stats df
    stats_df1 = pd.concat([total_count, dtl_count, from_date, to_date, min1, median1, mean1, max1, std1], axis=1)
...
def serve_layout():
    ...
    html.Div([
        ...
        html.Label('Site reference'),
        dcc.Dropdown(options=[], id='sites', multi=True),
    ...
    html.Div([
        html.P('Click on a site or select a group of sites (up to 10):', style={'display': 'inline-block'}),
        dcc.Graph(
            id = 'site-map',
...
#
    ...
    dash_table.DataTable(
        id='dtl_table',
        columns=[{"name": v, "id": v, 'deletable': True} for v in ['Site', 'Measurement', 'DTL value']],
        data=[],
        sort_action="native",
        sort_mode="multi",
        style_cell={
...
def update_ts_data(hts, parameter, site):
    # print(site)
    if (hts is None) or (parameter is None) or (len(site) == 0):
...
def update_display_map(site_data_str, figure):
    if site_data_str is None:
        ...
        fig = go.Figure(go.Scattermapbox(lat = lat1,
                                lon = lon1,
                                text = names1,
                                ids=names1,
                                hoverinfo = 'text',
...
def update_sites_values(selectedData, clickData):
    ...
    if selectedData:
        sel1 = [s['id'] for s in selectedData['points'] if 'id' in s]
        if sel1:
            site1_id = sel1[:10]
    elif clickData:
...
def update_summ_table(summ_data_str):
    if summ_data_str is not None:
        ...
        summ_table = summ_data.reset_index().to_dict('records')
...
@app.callback(
    Output('dtl_table', 'data'),
    [Input('dtl_data', 'data')],
    # [State('stn_dict', 'data'), State('method_dd', 'value'), State('active_select', 'value')]
    )
def update_dtl_table(dtl_data_str):
    if dtl_data_str is not None:
        dtl_data = decode_df(dtl_data_str).reset_index().drop('DateTime', axis=1).drop_duplicates()
        dtl_data.rename(columns={'Value': 'DTL value'}, inplace=True)
        dtl_table = dtl_data.to_dict('records')
...
def render_plot(tab, ts_data_str, dtl_data_str):

    # print(flow_stn_id)

    info_str = """
            ### Intro
            This is the [Environment Canterbury](https://www.ecan.govt.nz/) surface water quality dashboard for checking data quality.
    ...
    if tab == 'info_tab':
        ...
        if ts_data_str is not None:
            ts_data = decode_df(ts_data_str).reset_index()
            dtl_data = decode_df(dtl_data_str).reset_index().drop('DateTime', axis=1).drop_duplicates()
